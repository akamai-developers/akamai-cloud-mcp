...
async def test_list_ips(mock_get: None) -> None:
    data = await _call(build_server(domains="networking"), "list_ips")
    assert data["ips"][0]["address"] == "192.0.2.10"


async def test_list_vlans(mock_get: None) -> None:
    data = await _call(build_server(domains="networking"), "list_vlans")
    assert data["vlans"][0]["cidr"] == "10.0.0.0/24"


async def test_list_vpcs_has_no_subnets(mock_get: None) -> None:
    data = await _call(build_server(domains="networking"), "list_vpcs")
    vpc = data["vpcs"][0]
    assert vpc["id"] == 30
...
async def test_get_vpc_surfaces_subnets(mock_get: None) -> None:
    data = await _call(build_server(domains="networking"), "get_vpc", {"vpc_id": 30})
...
async def test_list_nodebalancers(mock_get: None) -> None:
    data = await _call(build_server(domains="networking"), "list_nodebalancers")
